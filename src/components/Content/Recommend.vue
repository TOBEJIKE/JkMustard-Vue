  ...
  export default {
    ...
    methods: {
      ...
      loadPageList:function () {
        this.$http.post('http://m.chinabyte.com/article/jiemo/list.do', {'classify':'all', 'pageNo':1, 'pageSize':this.pageSize}).then((response) => {
      ...
      more:function () {
        this.$http.post('http://m.chinabyte.com/article/jiemo/list.do', {'classify':'all', 'pageNo':this.pageNo, 'pageSize':this.pageSize}).then((response) => {
          if (response.data.code === 200) {
            console.log('------推荐----------')
            console.log(response.data.result.articleWapList)
